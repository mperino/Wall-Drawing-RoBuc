//Wall drawing RoBuc 
//Based on code from fumik.com - mail baank83@gmail.com
//Original Author: Nguyen Huu Phuc - 2022/May. All rights reserved.
//Wall RoBuc by Mark Perino mark.perino@gmail.com
...
void setup() {
  ...
  // setup Pixel Ring
  Serial.println("Clearing LED Ring");
  strip.begin();
  strip.show(); // Initialize all pixels to 'off'
  strip.setPixelColor(1, 0, 50, 0);
  strip.show(); // set green pixel status
  ...
  // which modes are we in Function Command vs Gcode, and INteractive vs. File
  pinMode(COMMAND_MODE_PIN, INPUT); // set defined pins to input
  pinMode(ACTION_MODE_PIN, INPUT); // set defined pins to input
  command_state = digitalRead(COMMAND_MODE_PIN);
  action_state = digitalRead(ACTION_MODE_PIN);
  if (command_state == HIGH) {
    Serial.println("Command Mode is GCODE");
    strip.setPixelColor(2, 0, 50, 0);
    strip.show(); // set pixel 2 to green aka gcode
  }
  else {
    Serial.println("Command Mode is Function Command. Aka original");
    strip.setPixelColor(2, 254, 119, 254);
    strip.show(); // set pixel 2 to Fuscia aka Function
  }
  if (action_state == HIGH) {
    Serial.println("Action Mode is Interactive");
    strip.setPixelColor(3, 5, 0, 50);
    strip.show(); // set pixel 3 to Indigo aka Interactive
  }
  else {
    Serial.println("Action Mode is File. draw.txt or draw.gcode will be used");
    strip.setPixelColor(3, 254, 119, 254);
    strip.show(); // set pixel 3 to Fuscia aka File
  }
  if (action_state == LOW) {
    ...
    Serial.println("Interactive Mode Setup.  Press Any Key to continue.");
...
void loop() {
    ...
    while (action_state == HIGH) {
      Serial.println("Your Command?: ");
      while (Serial.available() == 0) {   
      }
      draw_command = Serial.readString();
      draw_command.trim();
      execute_command(draw_command);
    }
...
void execute_command(String draw_command) {
  ...
  if (draw_command.startsWith("move_pen_abs")){
    ...
    x1 = x1v.toFloat();
    String y1v = values.substring(values.indexOf(",")+1,values.length());
    y1 = y1v.toFloat();
    move_pen_abs(x1,y1);  //move pen absolute to x1,y1
  //  Serial.println(draw_command);
  //  Serial.println(x1);
